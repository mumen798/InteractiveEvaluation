...
def evaluate(model, dataset, dataloader, tokenizer, opt):
    ...
    with torch.no_grad():
        for i, batch in enumerate(dataloader):
            ...
            scores = torch.stack(outputs.scores).transpose(1,0)
            ...
            for k in range(len(outputs.sequences)):
                total_entropy = 0
                for i in range(len(scores[k])):
                    logits = scores[k][i]
                    logits = softmax(logits).cpu().numpy()
                    log_logits = np.log(logits)
                    if np.isnan((-1 * np.sum(logits * log_logits)) / np.log(32128)):
                        total_entropy += 1
                    else:
                        total_entropy += (-1 * np.sum(logits * log_logits)) / np.log(32128)
                confidence = 1 - (total_entropy / (len(scores[k]) - 1))

                o = outputs.sequences[k]
                ...
                if opt.write_results:
                    obj = {"question": example["question"], "conv_id": example['conv_id'],
                           "turn_id": example['turn_id'], "predictions": [ans], "confidence": confidence}
...
if __name__ == "__main__":
    ...
    src.util.init_logger(logger, opt.is_main, opt.is_distributed, Path(opt.checkpoint_dir) / opt.name / 'run.log')
    for arg, value in vars(opt).items():
        logger.info("Input arg: %s: %r", arg, value)
    # if not directory_exists and opt.is_main:
    #     options.print_options(opt)

    tokenizer = transformers.T5TokenizerFast.from_pretrained('t5-base', return_dict=False)
