...
ReaderQuestionPredictions = collections.namedtuple(
    "ReaderQuestionPredictions", ["id", "conv_id", "turn_id", "predictions", "gold_answers", "confidence"]
)
...
class ReaderTrainer(object):
    ...
    def get_data_iterator(
        ...
        dataset = ExtractiveReaderDataset(
            ...
            self.cfg.encoder.sequence_length
        )
    ...
    def run_train(self):
        ...
        if cfg.local_rank in [-1, 0]:
            ...
            if self.best_cp_name is not None:
                shutil.copyfile(
                    self.best_cp_name,
    ...
    def _get_best_prediction(
        ...
        for q in range(questions_num):
            ...
            for p in range(passages_per_question):
                ...
                start_probs = softmax(torch.tensor(p_start_logits)).cpu().numpy()
                start_log_probs = np.log(start_probs)
                start_entropy = (-1 * np.sum(start_probs * start_log_probs)) / np.log(start_probs.shape[0])

                end_probs = softmax(torch.tensor(p_end_logits)).cpu().numpy()
                end_log_probs = np.log(end_probs)
                end_entropy = (-1 * np.sum(end_probs * end_log_probs)) / np.log(end_probs.shape[0])

                confidence = 1 - start_entropy * end_entropy
            ...
            if passage_thresholds:
                ...
                for n in passage_thresholds:
                    ...
                    for i in range(len(nbest)):
                        if nbest[i].passage_index < n:
                            curr_nbest.append(nbest[i])
                            curr_confidence.append(confidence_list[i])
                    if len(curr_nbest) == 0:
                        curr_nbest = [SpanPrediction("", -1, -1, -1, "")]
                    passage_rank_matches[n] = curr_nbest[0]
                    model_confidence = curr_confidence[0]
            ...
            batch_results.append(
                ReaderQuestionPredictions(sample.question, sample.conv_id, sample.turn_id, predictions, sample.answers, model_confidence)
            )
