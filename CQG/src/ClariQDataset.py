...
class ClariQDataset(Dataset):
    ...
    def example_to_tensor(self, idx):
        ...
        if len(input_seq) > self.hparams.max_seq_len:
            ...
            pad_num2 = self.hparams.max_seq_len - len(target)
            pad_num3 = self.hparams.max_seq_len - len(segments)
            pad_num4 = self.hparams.max_seq_len - len(mask)
            input_seq.extend([self.tokenizer.pad_token_id] * pad_num)
            # Target should be padded with -100s
            target.extend([-100] * pad_num2)
            segments.extend([self.tokenizer.pad_token_id] * pad_num3)
            mask.extend([0] * pad_num4)
    ...
    def build_input_from_segments(facets, history, question, tokenizer, lm_labels=False, with_eos=True, without_facets=False):
        ...
        if without_facets: # used for query-only baseline
            ...
            input_seq.append(tokenizer.bos_token_id)
            # input_seq += tokenizer.encode('<extra_id_0>')

        target_raw = question

        if isinstance(target_raw, str):
            # tmp = tokenizer.encode(target_raw)
            # tmp = tokenizer.encode('<extra_id_0>')
            tmp = tokenizer(text_target=target_raw)["input_ids"]
        else:
            tmp = target_raw
        if with_eos:
            tmp.append(tokenizer.eos_token_id)
            # tmp += tokenizer.encode('<extra_id_1>')
        ...
        # input_seq += tmp
        # input_seq += [tokenizer.pad_token_id] * len(tmp)
