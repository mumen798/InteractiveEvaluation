...
from neuroir.decoders.decoder import RNNDecoderBase
...
class RNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, state, memory_lengths=None):
        ...
        if isinstance(self.rnn, nn.GRU):
            ...
            # h0 = torch.cat((state.hidden[0], state.hidden[0]), dim=0)
            # c0 = torch.cat((state.hidden[1], state.hidden[1]), dim=0)
            # rnn_output, decoder_final = self.rnn(emb, (h0, c0))
