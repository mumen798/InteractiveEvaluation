...
def add_train_args(parser):
    ...
    ranker.add_argument('--model_type', type=str, default='seq2seq',
                        choices=['seq2seq', 'hredqs', 'acg'],
...
def init_from_scratch(args, train_exs, dev_exs):
    ...
    tgt_dict = utils.build_word_and_char_dict(args, train_exs + dev_exs,
                                              dict_size=args.tgt_vocab_size)
    logger.info('Num words in target vocabulary = %d' % len(tgt_dict))

    # Initialize model
    model = Recommender(config.get_model_args(args), src_dict, tgt_dict)
...
def train(args, data_loader, model, global_stats):
    ...
    pbar.set_description("%s" % 'Epoch = %d [ml_loss = x.xx]' % global_stats['epoch'])
...
def validate_official(args, data_loader, model, global_stats=None):
    ...
    if global_stats is None:
        logger.info('test results: rouge_l = %.2f | bleu = [%s] | ' %
        ...
        logger.info('dev results: Epoch = %d | rouge_l = %.2f | ' %
                    (global_stats['epoch'], result['rouge']) +
                    'bleu = [%s] | EM = %.2f | F1 = %.2f | examples = %d | ' %
                    (", ".join(format(b, ".2f") for b in bleu), result['em'],
                     result['f1'], examples) +
...
def main(args):
    ...
    if args.only_test:
        if args.pretrained:
            model = Recommender.load(args.pretrained, args)
        else:
            if not os.path.isfile(args.model_file):
                raise IOError('No such file: %s' % args.model_file)
            model = Recommender.load(args.model_file, args)
    else:
        if args.checkpoint and os.path.isfile(args.model_file + '.checkpoint'):
            # Just resume training, no modifications.
            logger.info('Found a checkpoint...')
            checkpoint_file = args.model_file + '.checkpoint'
            model, start_epoch = Recommender.load_checkpoint(checkpoint_file, args.cuda)
        else:
    ...
    if not args.only_test:
        train_dataset = data.RecommenderDataset(train_exs, model)
        if args.sort_by_len:
        ...
        dev_dataset = data.RecommenderDataset(dev_exs, model)
        if args.sort_by_len:
    ...
    if not args.only_test:
        ...
        model = Recommender.load(args.model_file)
